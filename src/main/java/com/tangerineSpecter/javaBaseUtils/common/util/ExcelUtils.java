...
import com.tangerinespecter.javabaseutils.common.exception.GlobalException;
...
public class ExcelUtils {
    ...
    private static List<String[]> getExcelForXlsx(String filePath) {
        try {
            ...
            e.printStackTrace();
            log.error(String.format("【Excel数据读取异常】：%s", e));
        }
        return null;
    }
     ...
     * @param tableHead 表头
     * @param dataList  数据列表
     * @param isExcel   （true:新版；false:旧版）
     * @return excel生成路径
     */
    ...
    private static void createExcelData(String[] tableHead, List<String[]> dataList) {
        try {
            ...
            log.error(String.format("【excel生成异常】：%s", e));
        } finally {
            if (wb != null) {
                try {
                    wb.close();
    ...
    public static List<List<Object>> readExcel(InputStream in) {
        ...
        try {
            ExcelUtil.readBySax(in, -1, (sheetIndex, rowIndex, rowList) -> {
                ...
                ArrayList<Object> objects = new ArrayList<>(rowList);
                result.add(objects);
            });
        } catch (Exception e) {
            log.error("读取EXCEL数据流失败...", e);
            throw new GlobalException("读取EXCEL数据流失败...", e);
        } finally {
            if (in != null) {
                try {
                    in.close();
